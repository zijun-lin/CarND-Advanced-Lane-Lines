...
class Line():
    def __init__(self, buff_num):
        ...
        self.recent_xfitted = []
        # the change of average x values of the fitted line over the last iterations
        self.bestx_change = None
    ...
    def process_params(self, is_fit_ok):
        ...
        if is_fit_ok:
            ...
            if self.bestx is None:
                ...
                last_bestx = np.array(self.bestx)  # save the last bestx value

            self.bestx = np.mean(self.recent_xfitted, axis=0)  # update the current bestx value

            self.bestx_change = self.bestx - last_bestx
            print('bestx_change: ', self.bestx_change[:5])
            self.best_fit = np.polyfit(y_pos, self.bestx, 2)
        else:
            print(' BBBBBBBBB ')
            self.bestx = self.bestx + self.bestx_change
            self.best_fit = np.polyfit(y_pos, self.bestx, 2)
    ...
    def update(self, x, y):
        ...
        self.process_params(self.sanity_check())
